        ...
        Pass
        {
            ...
            half4 hsv_to_rgb(float3 HSV)
            {
                half4 RGB = HSV.y;
                        ...
                        float var_1 = HSV.y * (1.0 - HSV.y);
                        float var_2 = HSV.y * (1.0 - HSV.y * (var_h - var_i));
                        float var_3 = HSV.y * (1.0 - HSV.y * (1 - (var_h - var_i)));
                        if (var_i == 0) { RGB = half4(HSV.y, var_3, var_1,1); }
                        else if (var_i == 1) { RGB = half4(var_2, HSV.y, var_1,1); }
                        else if (var_i == 2) { RGB = half4(var_1, HSV.y, var_3,1); }
                        else if (var_i == 3) { RGB = half4(var_1, var_2, HSV.y,1); }
                        else if (var_i == 4) { RGB = half4(var_3, var_1, HSV.y,1); }
                        else { RGB = half4(HSV.y, var_1, var_2, 1); }
            ...
            { 
                ...
                half4 col =  hsv_to_rgb(float3((i.localPos.z * _HueScale + 100000 + _HueShift) % 1.0, 1, 1));
